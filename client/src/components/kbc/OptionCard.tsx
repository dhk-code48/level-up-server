...
const OptionCard: FC<{ option: string; correct: boolean }> = ({ option, correct }) => {
  ...
  function handleOptionClick() {
    ...
    if (correct) {
      setbgColor("green");
      kbc.setDisableAllOption(true);
      kbc.setPauseGame(true);

      setTimeout(() => {
        kbc.steQuestionCounter((prev) => prev + 1);
        kbc.setDisableAllOption(false);
        kbc.setPauseGame(false);
        setbgColor("none");
      }, 3000);
    } else {
      setbgColor("red");
  ...
  return (
    <button onClick={handleOptionClick} disabled={kbc.disableAllOption}>
      <svg
        width="100%"
        height="60"
        viewBox="0 0 755 54"
